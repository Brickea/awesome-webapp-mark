...
var simulator = new Vue({
    ...
    mounted :function () {
        ...
        this.$nextTick(function () {
            ...
            // set options
            $( "#social_distance_switch" ).prop( "disabled", false  );
            $( "#mask_switch" ).prop( "disabled", false );
            $( "#testing_switch" ).prop( "disabled", false );
        })
        // create echart
    ...
    methods:{
        ...
        startSimulate: function(event) {
            // Reference : https://github.com/petrosDemetrakopoulos/SIR.js/blob/master/index.js
            ...
            if(this.simulationInterval==null){
                ...
                this.simulationInterval = setInterval(function () {
                    ...
                    var randomSI = Math.floor(Math.random() * 1000) + 1
                    var randomIR = Math.floor(Math.random() * 1000) + 1
...
function reset_history () {
    ...
    SIR_model_data.s_data=[[0,150]]
    SIR_model_data.i_data=[[0,0]]
    SIR_model_data.r_data=[[0,0]]

    if(SIR_model_data.myChart.setOption!=null){
        SIR_model_data.myChart.setOption({
            series:[
                {
                    name: SIR_model_data.s_name,
                    data: SIR_model_data.s_data
                },
                {
                    name: SIR_model_data.i_name,
                    data: SIR_model_data.i_data
                },
                {
                    name: SIR_model_data.r_name,
                    data: SIR_model_data.r_data
                }
            ]
        });
    }
...
function run_SIR() {
    ...
    $( "#social_distance_switch" ).prop( "disabled", true  );
    $( "#mask_switch" ).prop( "disabled", true );
    $( "#testing_switch" ).prop( "disabled", true );

    if($('#social_distance_switch').prop('checked'))
        p_SI = 0.15

    if($('#mask_switch').prop('checked'))
        p_SI = 0.1

    if($('#testing_switch').prop('checked')){
        p_SI = 0.05
        p_IR = 0.004
    }
    ...
    SIR_model_data.s_data.push([count,epi_state.S])
    SIR_model_data.i_data.push([count,epi_state.I])
    SIR_model_data.r_data.push([count,epi_state.R])
    SIR_model_data.myChart.setOption({
        series:[
            {
                name: SIR_model_data.s_name,
